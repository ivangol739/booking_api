...
@router.post('/')
async def create_booking(db: Annotated[AsyncSession, Depends(get_db)], booking_data: BookingCreate, user_id: int=1):
    ...
    slug = slugify(f"booking-{user_id}-{flight.id}-{datetime.utcnow().timestamp()}")
...
async def update_booking(db: Annotated[AsyncSession, Depends(get_db)], update_data: BookingUpdate, booking_slug: str):
    booking = await db.scalar(select(Booking).where(Booking.slug == booking_slug))
    if booking is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Booking not found')
...
@router.delete('/')
async def delete_booking(db: Annotated[AsyncSession, Depends(get_db)], booking_id: int):
